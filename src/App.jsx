...
import {
  ...
  Smile, X, Edit2, Trash2, Reply, Settings,
  Mic, Heart, Menu
} from 'lucide-react';
...
export default function App() {
  ...
  const [chats, setChats] = useState([
    ...
    },
    {
      ...
      unread: 0,
      messages: [
        { id: 1, sender: 'Мария', text: 'Привет всем!', time: 'Вчера', own: false, reactions: ['👋'] }
      ]
    },
    {
      id: 3,
      ...
      <div className={`flex-1 flex flex-col ${bg}`}>
        {selectedChat ? (
          <>
            ...
            <div className={`${bg} border-b ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'} p-4 flex justify-between items-center shadow-md`}>
              <div className="flex items-center gap-4">
                <button
                  ...
                  className={`p-2 hover:${secondary} rounded-lg`}
                >
              ...
              <div className="flex gap-2">
                <button className={`p-2 hover:${secondary} rounded-lg`}>
                ...
                <button className={`p-2 hover:${secondary} rounded-lg`}>
                  <MoreVertical size={20} className={text} />
                </button>
              </div>
            </div>
            ...
            <div className={`flex-1 overflow-y-auto p-6 space-y-4 ${secondary}`}>
              {selectedChat.messages.length === 0 ? (
                ...
                selectedChat.messages.map(msg => (
                  <div
                    key={msg.id}
                    className={`flex ${msg.own ? 'justify-end' : 'justify-start'} group`}
                  >
                    <div
                    ...
                    >
                      ...
                      <div className={`text-xs mt-2 ${msg.own ? 'text-blue-100' : theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}>
                        {msg.time}
                      </div>
                    ...
                    {selectedMessage === msg.id && (
                      ...
                      >
                        <button
                          ...
                          className={`p-2 rounded-lg hover:${secondary}`}
                        >
            ...
            {/* Поле ввода */}
            <div className={`${bg} border-t ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'} p-4 shadow-lg relative`}>
              <div className="flex gap-2 items-end">
                ...
                <button
                  ...
                  className={`p-2 rounded-lg ${recordingVoice ? 'bg-red-500 text-white' : `hover:${secondary}`}`}
                >
              ...
              {/* Пикер эмодзи */}
              {showEmojiPicker && (
                <div className={`absolute bottom-20 left-4 ${bg} rounded-2xl p-4 shadow-2xl border ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'} z-50 w-80`}>
                  <div className="grid grid-cols-6 gap-2 max-h-64 overflow-y-auto">
                    {EMOJIS.map((e, i) => (
                      <button
                        ...
                        className={`text-2xl p-2 hover:${secondary} rounded-lg`}
                      >
                        {e}
                      </button>
                    ))}
        ...
        <div className={`w-80 ${bg} border-l ${theme === 'dark' ? 'border-gray-700' : 'border-gray-200'} shadow-2xl flex flex-col`}>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            <div className={`${secondary} rounded-xl p-4 text-center`}>
              ...
              <button className="w-full py-2 bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-lg font-semibold mt-3">
                Редактировать
              </button>
            </div>
            ...
            <div className={`${secondary} rounded-xl p-4 flex items-center justify-between`}>
              ...
              <button
                ...
                className={`w-12 h-6 rounded-full transition ${theme === 'dark' ? 'bg-blue-600' : 'bg-gray-300'}`}
              >
            ...
            <div className={`${secondary} rounded-xl p-4`}>
              ...
              <button className={`w-full py-2 ${theme === 'dark' ? 'bg-gray-700' : 'bg-gray-200'} ${text} rounded-lg text-sm font-semibold`}>
                Звук: Вкл
              </button>
            </div>
            ...
            <button className="w-full py-3 bg-red-500 hover:bg-red-600 text-white rounded-lg font-semibold transition">
              🚪 Выход
            </button>
          </div>
        </div>
